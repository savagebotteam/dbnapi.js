const dbnRequest = require("./dbnRequest.js");
/**
 * discordbotsNats Client
 */
module.exports = class discordbotsNats {
    /**
     * Create new discordbotsNats Wrapper Client
     * @param {String} token discordbotsNats token
     * @param {String} clientID Your Bot's Client ID
     * @param {String} ownerID Your ID
     */
    constructor(token, clientID, ownerID) {
        this.baseURL = "discordbots-nats.glitch.me";
        this.baseAPIURL = this.baseURL + "/api";
        // if (!client.user.id) throw new Error("Unsupported Library")
        const request = new dbnRequest(this.baseURL);
        ...
        if (token || token !== undefined || token !== "") {
            tokenValidator(token, request).then(valid => {
                if (valid === "false") {
                    throw new Error("Invalid DiscordBots Nation API Token");
                    // eslint-disable-line
                } else {
                    if (valid === "true") {
                        fetchToken(token, clientID, ownerID, request).then(fetchedToken => {
                            ...
                            console.log(`[dbnAPI] You're now Logged in as: ${fetchedToken.ownedBy.tag}`);
        ...
        /**
         * dbnRequest Client for creating requests
         * @private
         */
        ...
        /**
         * Posts your Bot stats to DiscordBots Nation (serversCount and usersCount)
         * [!IMPORTANT!] This function needs discordbotsNats Token.
...
async function fetchToken(token, clientID, ownerID, request) { // eslint-disable-line
    ...
    var body = await response.body;
    if (body.valid === false) throw new Error("Invalid DiscordBots Nation API Token");
